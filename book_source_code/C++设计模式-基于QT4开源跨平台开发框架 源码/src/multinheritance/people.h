...
class GraduateTeachingFellow :
    public Student, public Teacher { /* virtual not needed
        here */
public:
    GraduateTeachingFellow(const Person& p,
                           const Student& s, const Teacher& t):
    Person(p), Students(s), Teacher(t) {}     /*  It 
    is necessary to initialize all
    virtual base classes explicitly in
    multiply-derived classes, to resolve
    ambiguity about how they should be
    initialized. */
}