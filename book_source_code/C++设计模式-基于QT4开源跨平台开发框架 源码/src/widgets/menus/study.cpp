...
QAction* Study::addChoice(QString name, QString text) {
    ...
    actionGroup->addAction(retval); /* Add every action we
    create to a QActionGroup so that we only connect
    one signal to a slot. */ 
    return retval;
}
//end
//start id=study
Study::Study() {
    ...
    QWidget::setWindowTitle( "to become a jedi, you wish?  " ); /* The ClassName:: prefixes we use in methods here are 
not necessary, because the methods can be called on "this".
We list the classname only to show the human reader from
which class the method was inherited. */
...
//start id=study
    ...
    QObject::connect(actionGroup, SIGNAL(triggered(QAction*)), 
            ...
            connecting each individual action's signal, we
